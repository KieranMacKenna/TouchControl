...
namespace Prime31
{
	...
	public class FacebookBinding
	{
		...
		#region iOS Facebook composer and Share Dialog
		...
		{
			if( Application.platform == RuntimePlatform.IPhonePlayer )
				_facebookShowFacebookShareDialog( Json.encode( parameters ) );
		}
		
		
		[DllImport("__Internal")]
		private static extern void _facebookShowFacebookShareDialogWithImage( string imagePath, string caption );
		
		// Shows the Facebook share dialog. Results in the shareDialogSucceeded/FailedEvent firing.
		public static void showFacebookShareDialogWithImage( string imagePath, string caption = "" )
		{
			if( Application.platform == RuntimePlatform.IPhonePlayer )
				_facebookShowFacebookShareDialogWithImage( imagePath, caption );
		}
		...
		[Obsolete( "Facebook has deprecated App Invites. This method will now no-op" )]
