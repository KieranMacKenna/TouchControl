...
namespace Prime31
{
	public class Facebook : P31RestKit
	{
		...
		public void getSessionPermissionsOnServer( Action<string, List<string>> completionHandler )
		{
			get( "me/permissions", ( error, obj ) =>
			{
				if( error == null && obj != null && obj is IDictionary )
				{
					...
					foreach( Dictionary<string,object> kv in dataDict )
					{
						if( kv["status"].ToString() == "granted" )
							grantedPermissions.Add( kv["permission"].ToString() );
					}
		...
		[System.Obsolete( "Facebook has killed all scores related functionality for new apps" )]
		public void postScore( int score, Action<bool> completionHandler )
		{
			...
			post( "me/scores", parameters, ( error, obj ) =>
			{
				...
				{
					// older Graph API versions return a string
					completionHandler( ((string)obj).ToLower() == "true" );
				}
				else if( error == null && obj != null && obj is IDictionary )
				{
					// newer Graph API versions return a dictionary/object
					var dict = obj as IDictionary;
					var result = (bool)dict["success"];
