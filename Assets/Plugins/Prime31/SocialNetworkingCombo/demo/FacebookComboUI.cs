...
namespace Prime31
{
	public class FacebookComboUI : MonoBehaviourGUI
	{
		...
		void OnGUI()
		{
			...
			if( GUILayout.Button( "Login/Reauth with Read Permissions" ) )
			{
			...
			if( GUILayout.Button( "Login/Reauth with Publish Permissions" ) )
			{
				var permissions = new string[] { "publish_actions" };
				FacebookCombo.loginWithPublishPermissions( permissions );
			}
