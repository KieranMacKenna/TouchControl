...
namespace Prime31
{
	public class TwitterAndroid
	{
		...
		// Sets up the Twitter Plugin for use.  When it is ready for use the twitterInitializedEvent will fire. Note that the
		// callbackUrl passed in must be present in your app in the Twitter developer portal.
		public static void init( string consumerKey, string consumerSecret, string callbackUrl = "twitterplugin" )
		{
			if( Application.platform != RuntimePlatform.Android )
				return;

			_plugin.Call( "init", consumerKey, consumerSecret, callbackUrl );
		}
		...
		// Shows the login dialog using an internal WebView
		...
		// Shows the login dialog using an external browser. Note that you must have properly setup your intent-filter
		// in your AndroidManifest to use an external browser for login! See the documentation for more information.
		public static void showLoginDialogViaExternalBrowser( string callbackUrlScheme )
		{
			if( Application.platform != RuntimePlatform.Android )
				return;

			_plugin.Call( "showLoginDialogViaExternalBrowser", callbackUrlScheme );
		}
		...
		public static void sendDirectMessage( string to, string message )
		{
			...
			_plugin.Call( "sendDirectMessage", to, message );
		}
